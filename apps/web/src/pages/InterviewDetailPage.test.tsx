...
function renderPage(initialEntries = ['/interview/typescript/what-is-a-closure']) {
...
describe('InterviewDetailPage', () => {
  it('falls back to the admin detail fetch when the public permalink returns 410', async () => {
    ...
    expect(screen.getByTestId('pathname')).toHaveTextContent(
      '/interview/typescript/what-is-a-closure',
    );
  });
  ...
  it('shows edit and restore for deleted posts and restores the active detail view on the same permalink', async () => {
    ...
    expect(await screen.findByRole('menuitem', { name: /Bearbeiten/i })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: /Wiederherstellen/i })).toBeInTheDocument();
    ...
    expect(screen.queryByRole('button', { name: /Wiederherstellen/i })).toBeNull();
    expect(screen.getByTestId('pathname')).toHaveTextContent(
      '/interview/typescript/what-is-a-closure',
    );
  });
