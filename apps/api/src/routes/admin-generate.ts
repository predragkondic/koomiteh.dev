...
import { generatePostRequestSchema, isLanguageId } from '@koomiteh/shared';
import { requireAdmin } from '../middleware/auth-context.js';
import { generatePostDraft } from '../services/post-generator.js';

export const adminGenerateRoute = new Hono();

adminGenerateRoute.use('*', requireAdmin);

adminGenerateRoute.post('/', async (c) => {
  let body: unknown;
  try {
