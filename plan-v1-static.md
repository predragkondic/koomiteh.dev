...
```tsx
<AppShell>                                 {/* Header: Logo, GlobalSearch ⌘K, LangToggle, ThemeToggle */}
  <Route index element={<Navigate to="/interview" replace />} />
  <Route path="interview" element={<InterviewLayout />}>   {/* Sprach-Tabs (versteckt bei 1 Sprache) */}
...
- Unbekannte Sprache / unbekannter Slug: NotFound *innerhalb* des jeweiligen Layouts (nicht App-weit).
