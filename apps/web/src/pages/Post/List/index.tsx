...
import { FilterBar, type FilterChange } from "@/components/FilterBar";
...
export function PostListingPage() {
  ...
  return (
    <Box>
      ...
      <ActiveFilterChips
        filter={filter}
        onRemoveLevel={() => writeState({ ...filter, level: "both", page: 1 })}
        onRemoveTag={(tag) =>
          writeState({
            ...filter,
        ...
        onRemoveSort={() => writeState({ ...filter, sort: "newest", page: 1 })}
...
function PostCard({ post }: { post: PostFrontmatter }) {
  ...
  const to = `/post/${post.language}/${post.slug}${search ? `?${search}` : ""}`;
