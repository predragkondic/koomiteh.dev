...
- **i18n:** `react-i18next`, `en` / `de`, namespaced (`common`, `post`), `localStorage` + Browser-Default, **nicht** in der URL.
...
- **Format:** Markdown + YAML Frontmatter, flat in `content/post/`.
- **Schema:**
  ```yaml
...
**Symmetrie zur späteren API:** `GET /content/indexes/{lang}.json` ist 1:1 das, was später `GET /api/post?language={lang}` liefern würde. Beim Switch ändert sich nur die Base-URL der RTK-Query-Endpoints.
...
<AppShell>
  ...
  <Route path="post" element={<InterviewLayout />}>
    {" "}
    {/* Sprach-Tabs (versteckt bei 1 Sprache) */}
    <Route index element={<InterviewHubPage />} /> {/* /post */}
...
- Single-Language-Sonderfälle: bei nur einer Sprache redirectet `/post` automatisch zur Sprach-Page; Sprach-Tabs werden nicht gerendert. Code-Pfad bleibt erhalten.
...
2. `useGetIndexQuery(language)` (oder alle Sprach-Indexe parallel auf `/post` ohne Sprache, sofern überhaupt geladen — Hub-Page lädt nicht).
...
- `react-i18next` mit Namespaces `common` und `post`.
